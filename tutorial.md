...
=== dev environment ===
AWS Account ID for dev (or 'skip'): 111111111111
Bootstrap IAM Role name for dev [TerraformBootstrapRole]: 
Terraform Execution Role name for dev [TerraformExecutionRole]: 

=== stage environment ===
AWS Account ID for stage (or 'skip'): skip

=== prod environment ===
AWS Account ID for prod (or 'skip'): skip
```
...
## 8) Add your own resources

1. Edit `infra/live/dev/storage/main.tf` or create a new stack
2. If creating a new stack, add it to the workflow matrix in `.github/workflows/terraform.yml`:
   ```yaml
   matrix:
     stack: [storage, newstack]
   ```
3. Open a PR to run plan, merge to apply
