...
#    - Merge to main → auto-deploys to dev, stage/prod wait for approval
```
...
#### Automatic Promotion with Approval Gates

When you merge to `main`, **all environments deploy automatically** with approval gates:

1. **Dev** - Deploys immediately (no approval)
2. **Stage** - Plan runs immediately, then waits for approval before apply
3. **Prod** - Plan runs immediately, waits for stage to complete, then waits for approval before apply

The workflow uses a **two-phase deployment** for stage/prod:

1. **Plan Phase** (automatic)
   - Runs `terraform plan` immediately on merge
   - Uploads plan artifact for review
   - Shows plan summary in workflow UI

2. **Apply Phase** (requires approval)
   - Waits for environment approval (configure in GitHub Settings)
   - Downloads the exact plan that was reviewed
   - Runs `terraform apply` with the saved plan

This ensures the **exact changes reviewed are what gets applied** - no drift between plan and apply.
...
| Merge to `main` | Deploys to **all environments** - dev auto-applies, stage/prod require approval |
...
### Environment Protection Rules (Required for Stage/Prod)

Configure approval gates in GitHub to enforce manual review before production deployments:

1. Go to **Settings** → **Environments**
2. Select `stage` or `prod` environment
3. Under **Environment protection rules**, click **Add rule**:

| Environment | Required Reviewers | Wait Timer | Branch Policy |
|-------------|-------------------|------------|---------------|
| `dev`       | None              | None       | `main` only   |
| `stage`     | 1+ team members   | 0-30 min   | `main` only   |
| `prod`      | 2+ senior engineers | 0-60 min | `main` only   |

**Required reviewers**: Approvers who must review and approve the deployment. They can view the plan output in the workflow summary before approving.

**Wait timer** (optional): Adds a delay before deployment can proceed, even after approval. Useful for coordinating with maintenance windows.

**Deployment branch policy**: Restrict which branches can deploy to the environment. Set to `main` to prevent feature branches from deploying to production.
